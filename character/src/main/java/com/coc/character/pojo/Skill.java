package com.coc.character.pojo;

/**
 * @author ran_ych
 * @create 2019-10-13  14:43
 * @desc
 */
public class Skill {
    private String userid;
    private String name;
    private String profession;
    private int hp;
    private int currenthp;
    private int mp;
    private int currentmp;
    private int san;
    private int currentsan;
    private int potential;
    private int interest;
    private int shin;
    private int swim;
    private int spy;
    private int listen;
    private int libaray;
    private int negotiate;
    private int occultism;
    private int cthulhu;
    private int drive;
    private int combat;
    private int gun;

    private int accountant;
    private int anthropology;
    private int evaluate;
    private int archeology;
    private int charm;
    private int computer;
    private int credit;
    private int dress;
    private int dodge;
    private int electrical;
    private int mechanology;
    private int law;
    private int locksmith;
    private int sneak;
    private int medical;
    private int foreign;
    private int pilot;
    private int psychology;
    private int trace;
    private int aid;
    private int heavy;
    private int steal;
    private int threaten;
    private int chemistry;




    public String getUserid() {
        return userid;
    }

    public void setUserid(String userid) {
        this.userid = userid;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getProfession() {
        return profession;
    }

    public void setProfession(String profession) {
        this.profession = profession;
    }

    public int getHp() {
        return hp;
    }

    public void setHp(int hp) {
        this.hp = hp;
    }

    public int getCurrenthp() {
        return currenthp;
    }

    public void setCurrenthp(int currenthp) {
        this.currenthp = currenthp;
    }

    public int getMp() {
        return mp;
    }

    public void setMp(int mp) {
        this.mp = mp;
    }

    public int getCurrentmp() {
        return currentmp;
    }

    public void setCurrentmp(int currentmp) {
        this.currentmp = currentmp;
    }

    public int getSan() {
        return san;
    }

    public void setSan(int san) {
        this.san = san;
    }

    public int getCurrentsan() {
        return currentsan;
    }

    public void setCurrentsan(int currentsan) {
        this.currentsan = currentsan;
    }

    public int getPotential() {
        return potential;
    }

    public void setpotential(int potential) {
        this.potential = potential;
    }

    public int getInterest() {
        return interest;
    }

    public void setInterest(int interest) {
        this.interest = interest;
    }

    public int getShin() {
        return shin;
    }

    public void setShin(int shin) {
        this.shin = shin;
    }

    public int getSwim() {
        return swim;
    }

    public void setSwim(int swim) {
        this.swim = swim;
    }

    public int getSpy() {
        return spy;
    }

    public void setSpy(int spy) {
        this.spy = spy;
    }

    public int getListen() {
        return listen;
    }

    public void setListen(int listen) {
        this.listen = listen;
    }

    public int getLibaray() {
        return libaray;
    }

    public void setLibaray(int libaray) {
        this.libaray = libaray;
    }

    public int getNegotiate() {
        return negotiate;
    }

    public void setNegotiate(int negotiate) {
        this.negotiate = negotiate;
    }

    public int getOccultism() {
        return occultism;
    }

    public void setOccultism(int occultism) {
        this.occultism = occultism;
    }

    public int getCthulhu() {
        return cthulhu;
    }

    public void setCthulhu(int cthulhu) {
        this.cthulhu = cthulhu;
    }

    public int getDrive() {
        return drive;
    }

    public void setDrive(int drive) {
        this.drive = drive;
    }

    public int getCombat() {
        return combat;
    }

    public void setCombat(int combat) {
        this.combat = combat;
    }

    public int getGun() {
        return gun;
    }

    public void setGun(int gun) {
        this.gun = gun;
    }

    public void setPotential(int potential) {
        this.potential = potential;
    }

    public int getAccountant() {
        return accountant;
    }

    public void setAccountant(int accountant) {
        this.accountant = accountant;
    }

    public int getAnthropology() {
        return anthropology;
    }

    public void setAnthropology(int anthropology) {
        this.anthropology = anthropology;
    }

    public int getEvaluate() {
        return evaluate;
    }

    public void setEvaluate(int evaluate) {
        this.evaluate = evaluate;
    }

    public int getArcheology() {
        return archeology;
    }

    public void setArcheology(int archeology) {
        this.archeology = archeology;
    }

    public int getCharm() {
        return charm;
    }

    public void setCharm(int charm) {
        this.charm = charm;
    }

    public int getComputer() {
        return computer;
    }

    public void setComputer(int computer) {
        this.computer = computer;
    }

    public int getCredit() {
        return credit;
    }

    public void setCredit(int credit) {
        this.credit = credit;
    }

    public int getDress() {
        return dress;
    }

    public void setDress(int dress) {
        this.dress = dress;
    }

    public int getDodge() {
        return dodge;
    }

    public void setDodge(int dodge) {
        this.dodge = dodge;
    }

    public int getElectrical() {
        return electrical;
    }

    public void setElectrical(int electrical) {
        this.electrical = electrical;
    }

    public int getMechanology() {
        return mechanology;
    }

    public void setMechanology(int mechanology) {
        this.mechanology = mechanology;
    }

    public int getLaw() {
        return law;
    }

    public void setLaw(int law) {
        this.law = law;
    }

    public int getLocksmith() {
        return locksmith;
    }

    public void setLocksmith(int locksmith) {
        this.locksmith = locksmith;
    }

    public int getSneak() {
        return sneak;
    }

    public void setSneak(int sneak) {
        this.sneak = sneak;
    }

    public int getMedical() {
        return medical;
    }

    public void setMedical(int medical) {
        this.medical = medical;
    }

    public int getForeign() {
        return foreign;
    }

    public void setForeign(int foreign) {
        this.foreign = foreign;
    }

    public int getPilot() {
        return pilot;
    }

    public void setPilot(int pilot) {
        this.pilot = pilot;
    }

    public int getPsychology() {
        return psychology;
    }

    public void setPsychology(int psychology) {
        this.psychology = psychology;
    }

    public int getTrace() {
        return trace;
    }

    public void setTrace(int trace) {
        this.trace = trace;
    }

    public int getAid() {
        return aid;
    }

    public void setAid(int aid) {
        this.aid = aid;
    }

    public int getHeavy() {
        return heavy;
    }

    public void setHeavy(int heavy) {
        this.heavy = heavy;
    }

    public int getSteal() {
        return steal;
    }

    public void setSteal(int steal) {
        this.steal = steal;
    }

    public int getThreaten() {
        return threaten;
    }

    public void setThreaten(int threaten) {
        this.threaten = threaten;
    }

    public int getChemistry() {
        return chemistry;
    }

    public void setChemistry(int chemistry) {
        this.chemistry = chemistry;
    }

    private boolean isBasisRangeSkill(int skill){
        if(skill>70&&skill<0){
            return false;
        }else{
            return true;
        }
    }
}
